...
const CategorySeoPage = () => {
  ...
  // Fetch all category pages on component mount
  useEffect(() => {
    fetchPages()
  }, [])
  ...
  const fetchPages = async () => {
    try {
      ...
      if (response.status === 200 && Array.isArray(response.data?.data)) {
        ...
        // If data format is different or empty, handle gracefully
        if (response.data && !response.data.data) {
          setPages([]); // Assume empty if no data array
        } else {
      ...
      setError("Failed to load category pages. Please try again.");
    } finally {
      setLoading(false);
    }
  };
    ...
    try {
      ...
      if (selectedPage._id.startsWith('new-')) {
        ...
        if (response.data) {
          ...
          setSelectedPage(newPage)
          setIsEditing(false)
          showToast('Category SEO created successfully!', 'success')
        }
      } else {
        ...
        if (response.data) {
          ...
          setIsEditing(false)
          showToast('Category SEO updated successfully!', 'success')
        }
      }
    } catch (err) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 p-3 md:p-6 pb-24 md:pb-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-gray-600 text-xs md:text-sm mt-0.5">
                Manage SEO metadata for product categories
              </p>
            </div>
          </div>
      ...
      {showMobileMenu && (
        <div className="lg:hidden fixed inset-0 bg-black/60 backdrop-blur-sm z-50" onClick={() => setShowMobileMenu(false)}>
          <div className="absolute right-0 top-0 h-full w-80 max-w-[85vw] bg-white shadow-2xl p-5 overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            ...
            {/* Mobile Search */}
            <div className="mb-4">
              <label className="block text-xs font-semibold text-gray-700 mb-2 uppercase tracking-wide">Search Categories</label>
            ...
            <button
              ...
              disabled={loading}
              className="w-full mb-6 bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700 text-white text-sm font-medium px-4 py-3 rounded-lg flex items-center justify-center gap-2 transition-all shadow-md hover:shadow-lg disabled:opacity-50"
            >
            ...
            <div className="space-y-3">
              <p className="text-xs font-semibold text-gray-500 uppercase tracking-wider">Categories ({filteredPages.length})</p>
            ...
            <div className="bg-white rounded-xl border border-gray-200 p-3 md:p-4 shadow-sm hover:shadow-md transition-shadow">
              <div className="flex flex-col">
                <div className="flex items-center justify-between mb-2">
                  <Layout className="w-6 h-6 md:w-8 md:h-8 text-purple-500" />
                </div>
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              ...
              <div className="flex gap-2">
                ...
                <button
                  ...
                  disabled={loading}
                  className="bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700 text-white text-sm font-medium px-4 py-2.5 rounded-lg flex items-center gap-2 transition-all shadow-sm hover:shadow-md disabled:opacity-50 whitespace-nowrap"
                >
                  <Plus className="w-4 h-4" />
                  Add Category
                </button>
          ...
          <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden hidden lg:block">
            {loading ? (
              <div className="flex justify-center items-center py-16">
                ...
                <span className="ml-3 text-sm text-gray-500 font-medium">Loading categories...</span>
              </div>
            ) : (
              <>
                <div className="overflow-x-auto">
                  <table className="w-full">
                    <thead className="bg-gradient-to-r from-gray-50 to-gray-100 border-b border-gray-200">
                      <tr>
                        <th className="py-3.5 px-4 text-left text-xs font-bold text-gray-700 uppercase tracking-wider">Category Details</th>
                    ...
                    <tbody className="divide-y divide-gray-200">
                      {filteredPages.map((page) => (
                        <tr
                          key={page._id}
                          className={`hover:bg-purple-50/50 cursor-pointer transition-colors ${selectedPage && selectedPage._id === page._id ? 'bg-purple-50' : ''
                            }`}
                          ...
                          <td className="py-3.5 px-4">
                            <div className="flex items-center gap-1.5">
                              ...
                              <button
                                onClick={(e) => {
                                  e.stopPropagation()
                                  if (confirm(`Delete category "${page.pageTitle}"?`)) {
                ...
                {filteredPages.length === 0 && (
                  <div className="text-center py-12">
                    ...
                    <p className="text-gray-500 text-sm font-medium mb-1">
                      {searchTerm || statusFilter !== 'all' ? 'No categories found matching your criteria' : 'No categories available'}
                    </p>
                    <p className="text-gray-400 text-xs mb-4">Start by adding a new category type</p>
                    <button
                      onClick={handleAddNew}
          ...
          {selectedPage ? (
            <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-4 md:p-6 lg:sticky lg:top-6">
              <div className="flex justify-between items-start mb-6 pb-4 border-b border-gray-200">
                <div>
                  <h2 className="text-lg md:text-xl font-bold text-gray-900">
                    {isEditing ? (selectedPage._id.startsWith('new-') ? 'New Category SEO' : 'Edit Category SEO') : 'Category Details'}
                  </h2>
                ...
                <div className="flex gap-2">
                  {isEditing ? (
                    ...
                    <button
                      ...
                      className="px-3 md:px-4 py-2 text-sm font-medium bg-gradient-to-r from-purple-600 to-pink-600 text-white rounded-lg hover:from-purple-700 hover:to-pink-700 flex items-center gap-2 transition-all shadow-sm hover:shadow-md"
                    >
                      <Edit className="w-4 h-4" />
                      Edit
                    </button>
              ...
              <div className="space-y-5">
                  ...
                  <div>
                    ...
                    {isEditing ? (
                      <input
                        ...
                        placeholder="Enter category name (e.g., Watches)"
                        required
                      />
                    ) : (
                      <div className="text-sm text-gray-900 p-3 bg-gray-50 rounded-lg font-medium border border-gray-100">{selectedPage.pageTitle || 'No category name'}</div>
                    )}
                  </div>
                    ...
                    {isEditing ? (
                      <div>
                        <input
                          ...
                          placeholder="Enter slug (e.g., /watches)"
                          required
                        />
                ...
                {/* SEO Preview */}
                <div className="border-2 border-dashed border-gray-300 rounded-xl p-4 bg-gradient-to-br from-gray-50 to-purple-50">
                  ...
                  <div className="space-y-2">
                    <div className="text-blue-700 text-base font-medium truncate hover:underline cursor-pointer">
                      {isEditing ? (editingData.seoTitle || 'Category SEO Title') : (selectedPage.seoTitle || 'No SEO title')}
                    </div>
                    <div className="text-emerald-700 text-sm break-all">
                      https://www.montres.ae{isEditing ? (editingData.slug ? (editingData.slug.startsWith('/') ? editingData.slug : `/${editingData.slug}`) : '/category-slug') : (selectedPage.slug ? (selectedPage.slug.startsWith('/') ? selectedPage.slug : `/${selectedPage.slug}`) : '/category-slug')}
                    </div>
            ...
            <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-8 md:p-12 text-center">
              <div className="max-w-sm mx-auto">
                ...
                <h3 className="text-lg md:text-xl font-bold text-gray-900 mb-2">No Category Selected</h3>
                <p className="text-gray-500 text-sm mb-6">
                  Select a category from the list to view and edit SEO content, or add a new one.
                </p>
                <button
                  onClick={handleAddNew}
                  disabled={loading}
                  className="inline-flex items-center gap-2 bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700 text-white text-sm font-medium px-6 py-3 rounded-lg transition-all shadow-md hover:shadow-lg disabled:opacity-50"
                >
                  <Plus className="w-4 h-4" />
                  Add New Category
                </button>
      ...
      <div className="lg:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 p-3 shadow-2xl z-40">
        <div className="flex justify-between items-center gap-3">
          <div className="flex-1 min-w-0">
            <div className="text-xs text-gray-500 font-medium">Selected Category</div>
            <div className="text-sm font-semibold text-gray-900 truncate">
              {selectedPage?.pageTitle || 'No category selected'}
            </div>
          </div>
          <div className="flex gap-2">
